...
END{
   ...
   for (opt in tree) if (opt != "") {
      ...
      split(requires[opt] iffs[opt] enabledby[opt], radius)
      for (i in radius) list[radius[i]] = 1
   ...
   while (npending > 0) {
      ...
      if (option[opt] == "on") {
         nreqs = split(requires[opt], radius)
         for (j=1; j<=nreqs; ++j) {
            if (option[radius[j]] == "off") {
               print "option", opt, "turned on, but requirement", radius[j], "is turned off"
               err = 1
            } else if (option[radius[j]] != "on") {
               option[radius[j]] = "on"
               pending[++npending] = radius[j]
            }
         }
      } else {
         if (option[opt] != "off") {
         ...
         nreqs = split(enabledby[opt], radius)
         for (j=1; j<=nreqs; ++j) {
            if (option[radius[j]] == "on") {
               print "option", opt, "turned off, but enabled by", radius[j], "which is turned on"
               err = 1
            } else if (option[radius[j]] != "off") {
               option[radius[j]] = "off"
               pending[++npending] = radius[j]
            }
         }
   ...
   while (!finished) {
      ...
      for (i in option) if (!done[i]) {
         nreqs = split(tree[i], radius)
         if (nreqs > 0) {
            for (j=1; j<=nreqs; ++j) if (!done[radius[j]]) {
               break
            }
            if (j<=nreqs) {
               finished = 0
         ...
         # requires
         nreqs = split(requires[i], radius)
         for (j=1; j<=nreqs; ++j) {
            print "#ifndef PNG_" radius[j] "_SUPPORTED" >out
            print "#   undef PNG_on /*!" radius[j] "*/" >out
            ...
            if (option[i] == "on") {
               print error, i, "requires", radius[j] end >out
            }
            print "#endif" >out
         }
         ...
         if (nreqs > 0) {
            ...
            for (j=1; j<=nreqs; ++j) {
               print "#ifdef PNG_" radius[j] "_SUPPORTED" >out
               print "#   undef PNG_no_if /*" radius[j] "*/" >out
               print "#endif" >out
            }
         ...
         print "   /* enabled by" enabledby[i], "*/" >out
         nreqs = split(enabledby[i], radius)
         for (j=1; j<=nreqs; ++j) {
            print "#ifdef PNG_" radius[j] "_SUPPORTED" >out
            print "#   undef PNG_not_enabled /*" radius[j] "*/" >out
            # Oops, probably not intended (should be factored
            # out by the checks above).
            if (option[i] == "off") {
               print error, i, "enabled by:", radius[j] end >out
            }
            print "#endif" >out
         }
         ...
         if (i ~ /^fail_/) {
            ...
            nreqs = split(sets[i], radius)
            for (j=1; j<=nreqs; ++j) {
               print "#    ifdef PNG_set_" radius[j] >out
               # Some other option has already set a value:
               print error, i, "sets", radius[j] ": duplicate setting" end >out
               print error, "   previous value: " end "PNG_set_" radius[j] >out
               print "#    else" >out
               ...
               # a name or a string.
               print "#     define PNG_set_" radius[j], setval[i " " radius[j]] >out
               print "#    endif" >out
            }
         }
      ...
      if (!finished && !movement) {
         ...
         for (i in option) if (!done[i]) {
            ...
            nreqs = split(tree[i], radius)
            if (nreqs > 0) for (j=1; j<=nreqs; ++j) if (!done[radius[j]]) {
               print "   " radius[j]
            }
         }
         exit 1
      ...
      for (i in setting) if (!doneset[i]) {
         ...
         for (j=1; j<=nreqs; ++j) {
            if (option[radius[j]] != "")
               ...
               print "#ifndef PNG_" radius[j] >out
            print error, i, "requires", radius[j] end >out
            print "# endif" >out
         }
         # The precedence is:
